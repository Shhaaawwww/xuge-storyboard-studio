...
function characterFor(name: string, index: number): Character {
  return {
    id: `character_${index + 1}`,
    name,
    role: index === 0 ? "主要人物" : "故事人物",
    appearance: ["原文未明确，生成前需要用户确认"],
    personality: ["由行动与对白进一步确认"],
    visualMotifs: [index === 0 ? "与故事核心物件形成视觉联系" : "保持跨格可识别特征"],
    lockedFacts: []
  };
}
...
export class DemoProvider implements NarrativeProvider {
  ...
  async buildStoryBible(input: ValidPipelineRequest): Promise<StoryBible> {
    ...
    const characters = (names.length ? names : ["叙述者"]).map(characterFor);
    return {
      logline: cleanExcerpt(sentences.slice(0, 2).join(""), 110),
      themes: input.mode === "faithful" ? ["记忆", "事件还原"] : ["记忆", "变化", "人与空间"],
      narrativeVoice: /我|我们/.test(input.sourceText) ? "第一人称" : "第三人称",
      characters,
      locations: [{
        id: "location_1",
        name: /故乡|老屋|家/.test(input.sourceText) ? "故事中的旧居与周边" : "主要场景",
        description: "依据原文建立空间关系，未明确的细节需要在视觉设定阶段补充。",
        fixedElements: ["保持出入口、主要家具与标志性物件的位置连续"]
      }],
      ...
      ambiguities: ["人物精确外貌尚未由原文确认", "天气、年代和空间布局需要用户确认"]
    };
  }
  ...
  async buildAdaptation(input: ValidPipelineRequest, bible: StoryBible): Promise<AdaptationPlan> {
    ...
    return {
      approach: modeText,
      pacing: `${input.panelCount} 格结构：开场建立环境，中段推进事件，结尾保留情绪余韵。`,
      visualStrategy: `${input.style}；使用远景建立空间、中近景表现动作、特写呈现情绪与关键物件。`,
      decisions: bible.timeline.slice(0, input.panelCount).map((event, index) => ({
        id: `decision_${index + 1}`,
        source: event.sourceExcerpt,
        decision: index === 0 ? "作为建立镜头" : index === bible.timeline.length - 1 ? "作为情绪收束" : "转化为可见动作",
        reason: "保证每格承担一个明确叙事功能",
  ...
  async buildStoryboard(input: ValidPipelineRequest, bible: StoryBible): Promise<PromptCard[]> {
    ...
    return Array.from({ length: input.panelCount }, (_, index) => {
      ...
      const location = /车站|站台|雨棚/.test(source)
        ? "故乡车站"
        : /河边|旧路/.test(source)
          ? "河边旧路"
      ...
      const purpose = isFirst ? "建立时间、地点与人物处境" : isLast ? "完成事件并留下情绪余韵" : "推进动作并呈现人物反应";
      return {
        ...
        location,
        action,
        emotion: isFirst ? "克制、略带迟疑" : isLast ? "安静、复杂的余韵" : "专注、逐渐变化",
        shotSize,
        cameraAngle,
        composition: isFirst ? "人物置于画面边缘，环境占据大部分画面" : "主体与关键物件形成清晰的前后景关系",
        lighting: "统一的柔和环境光，关键区域有明确明暗层次",
        continuity: [
          `人物外观严格引用 ${characters[0] || "主角"} 的角色设定`,
          "场景固定物件保持位置连续",
          "服装、时间与天气承接上一格"
        ],
        prompt: `${input.style}。${shotSize}，${cameraAngle}。${panelCharacters.join("与") || "环境空镜"}，${action}。${isFirst ? "人物位于画面边缘，环境建立明确空间关系" : "人物动作清晰，主体与关键物件形成前后景"}。${creative ? "通过玻璃倒影和留白表现记忆与现实的重叠。" : "情绪通过姿态、视线和环境细节表达。"}统一角色设计，连续的服装与场景，柔和环境光，叙事性构图，无文字。`,
        negativePrompt: "文字，水印，签名，多余人物，重复肢体，错误手指，人物外观漂移，服装变化，场景结构矛盾，过度拥挤的构图",
  ...
  async audit(input: ValidPipelineRequest, bible: StoryBible, panels: PromptCard[]): Promise<AuditReport> {
    ...
    if (bible.ambiguities.length) {
      issues.push({
        ...
        message: "人物外貌和部分环境信息没有原文依据。",
        suggestion: "正式生成前补充角色卡，或明确授权系统进行视觉创作。"
      });
    }
    if (input.panelCount > 14) {
      issues.push({
        id: "audit_2", severity: "P2" as const, target: "Storyboard",
        message: "短文本被拆成较多画格，部分镜头可能重复。",
        suggestion: "合并相邻动作，或增加环境空镜承担节奏功能。"
      });
    }
    return {
      score: issues.length ? 86 : 94,
      summary: "分镜已覆盖主要事件，Prompt 结构完整；正式出图前建议确认未明确的人物视觉设定。",
      issues,
  ...
  async proposeEdit(
    ...
    if (target === "bible") {
      ...
      bible.ambiguities = [...bible.ambiguities, `用户编辑要求：${instruction}`];
    ...
    return {
      target,
      summary: "已把用户要求整理为可应用的结构化修改",
      rationale: "演示模式使用确定性规则生成建议；连接模型后会给出更精细的字段级调整。",
      changes: [{ field: target, before: "当前版本", after: instruction, reason: "响应用户编辑要求" }],
      revisedArtifact
    };
  }
}
